...
class ClassUpdate(BaseManager):
    def get(self):
        ...
        Class = models.Class.query.filter_by(id=params.get('Class_id')).first()
        return render.Class_update(Class=Class)

    def post(self):
        params = request.form
        Class = models.Class.query.filter_by(id=params.get('id')).first()
        if not Class:
            return exception2json("Class is not exist")
        values = {
            "name": params.get('name'),
